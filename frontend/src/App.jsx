...
function NotificationBell({ user }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }} title={`${unread} unread notification${unread === 1 ? '' : 's'}`}>
      ...
      {unread > 0 && (
        <span style={{ position: 'absolute', top: -6, right: -8, minWidth: 15, height: 15, borderRadius: 8, background: 'var(--accent-red)', color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>
          {unread}
        </span>
      )}
    </div>
...
function Layout({ user, onLogout, children }) {
  ...
  return (
    <div className="app-layout">
      <Analytics />
      <Sidebar user={user} onLogout={onLogout} drawerOpen={drawerOpen} onDrawerClose={() => setDrawerOpen(false)} appearance={isLightAdminPage ? 'light' : 'dark'} />
      <main className={`app-main${isLightAdminPage ? ' app-main-overview-light' : ''}`} style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', ...mainTheme }}>
        <div className="topbar-mobile">
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <button className="btn-ghost hamburger-btn" onClick={() => setDrawerOpen(true)} aria-label="Open menu" style={{ padding: 6, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Menu size={20} />
            </button>
            <div style={{ width: 30, height: 30, borderRadius: 8, background: isLightAdminPage ? '#2563eb' : 'linear-gradient(135deg, #0ea5e9, #14b8a6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: '#fff', fontSize: 14, fontFamily: 'var(--font-display)', flexShrink: 0 }}>C</div>
            <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: isLightAdminPage ? '#f4f4f5' : '#0ea5e922', border: `1px solid ${isLightAdminPage ? '#e4e4e7' : '#0ea5e955'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: isLightAdminPage ? '#52525b' : 'var(--accent-blue)', fontSize: 11, fontWeight: 600, fontFamily: 'var(--font-display)' }}>{initial}</div>
            <span className={`badge badge-${user?.role}`} style={{ fontSize: 10 }}>{user?.role}</span>
          </div>
        </div>

        {user?.role === 'admin' && <AdminTopNavbar user={user} onLogout={onLogout} />}
        ...
        <div className="app-main-scroll" style={{ flex: 1, overflowY: 'auto' }}>
          {user?.role !== 'admin' && (
            <div className="topbar-desktop">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 12px' }}>
                <NotificationBell user={user} />
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#0ea5e922', border: '1px solid #0ea5e955', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-blue)', fontSize: 11, fontWeight: 600, fontFamily: 'var(--font-display)' }}>{initial}</div>
          ...
          <div style={{ marginTop: 40, paddingTop: 16, borderTop: '1px solid var(--border)', textAlign: 'center', color: 'var(--text-muted)', fontSize: 11 }}>
            Built by <a href="https://baronsdigital.com" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-blue)', fontWeight: 600, textDecoration: 'none' }}>Barons Digital</a>
          </div>
        </div>
      </main>
      <Toast />
...
export default function App() {
  const activationRequested = new URLSearchParams(window.location.search).get('activate') === '1';
  const [user, setUser] = useState(null);
  const [loading, setLoading] = useState(true);
  const [authView, setAuthView] = useState(activationRequested ? 'signup' : 'login');
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'var(--bg-base)' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: 48, height: 48, borderRadius: 14, background: 'linear-gradient(135deg, #0ea5e9, #14b8a6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, fontWeight: 900, color: '#fff', margin: '0 auto 16px', fontFamily: 'var(--font-display)' }}>C</div>
  ...
  if (ENABLE_DESIGN_SYSTEM && window.location.pathname === '/design-system') {
    return (
      <BrowserRouter>
        <Routes>
          <Route path="/design-system" element={<Suspense fallback={<div style={{ minHeight: '100vh', background: '#020617' }} />}><DesignSystem /></Suspense>} />
