...
import { ForgotPassword, ResetPassword } from './views/PasswordRecovery';
...
function NotificationBell({ user }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }} title={`${unread} unread notification${unread === 1 ? '' : 's'}`}>
      ...
      {unread > 0 && (
        <span style={{ position: 'absolute', top: -6, right: -8, minWidth: 15, height: 15, borderRadius: 8, background: 'var(--accent-red)', color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>{unread}</span>
      )}
    </div>
  );
}
...
function Layout({ user, onLogout, children }) {
  ...
  const mainTheme = isLightAdminPage ? {
    background: '#fafafa', '--bg-base': '#fafafa', '--bg-surface': '#ffffff', '--bg-card': '#ffffff',
    '--bg-input': '#ffffff', '--border': '#e4e4e7', '--border-hover': '#d4d4d8', '--text-primary': '#18181b',
    '--text-secondary': '#52525b', '--text-muted': '#71717a', '--accent-blue': '#2563eb', '--accent-teal': '#0f766e',
        ...
        <div className="app-main-scroll" style={{ flex: 1, overflowY: 'auto' }}>
          {user?.role !== 'admin' && (
            <div className="topbar-desktop">
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 14px', color: 'var(--text-muted)', fontSize: 12 }}>📅 {MONTHS[now.getMonth()]} {now.getFullYear()}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 12px' }}><NotificationBell user={user} /><div style={{ width: 28, height: 28, borderRadius: '50%', background: '#0ea5e922', border: '1px solid #0ea5e955', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-blue)', fontSize: 11, fontWeight: 600 }}>{initial}</div><span style={{ color: 'var(--text-primary)', fontSize: 12, fontWeight: 600 }}>{user?.name || user?.username}</span></div>
            </div>
          )}
          {children}
          <div style={{ marginTop: 40, paddingTop: 16, borderTop: '1px solid var(--border)', textAlign: 'center', color: 'var(--text-muted)', fontSize: 11 }}>Built by <a href="https://baronsdigital.com" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-blue)', fontWeight: 600, textDecoration: 'none' }}>Barons Digital</a></div>
        </div>
      </main>
      <Toast />
    </div>
...
export default function App() {
  ...
  const activationRequested = params.get('activate') === '1';
  ...
  if (!user) {
    ...
    if (authView === 'signup') return <SignUp onLogin={setUser} onSwitchToLogin={returnToLogin} />;
    return <Login onLogin={setUser} onSwitchToSignup={() => setAuthView('signup')} onForgotPassword={() => setAuthView('forgot')} />;
  }
