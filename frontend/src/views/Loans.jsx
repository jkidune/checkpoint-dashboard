      ...
      <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>
          {[
            ...
            ['Interest (5%)', fmt(data.interest_amount), 'var(--accent-amber)'],
        ...
        <ProgressBar value={data.total_repaid} max={data.principal}/>
...
export default function Loans({ user }) {
  ...
  const [form, setForm] = useState({ member_id:'', principal:'', issued_date:'', due_date:'', fiscal_year:'2025', notes:'' });
  const [saving, setSaving] = useState(false);
  ...
  const handleAddLoan = async (e) => {
    ...
    try {
      ...
      setShowAdd(false);
      setForm({ member_id:'', principal:'', issued_date:'', due_date:'', fiscal_year:'2025', notes:'' });
      refetch();
    } catch(e) {
      showToast(e.response?.data?.error || 'Failed', 'error');
    } finally { setSaving(false); }
  ...
  return (
    <div className="page">
      <SectionHeader title="Loans" sub={`FY${fiscalYear} · ${list.length} loans`}
        action={
          <div style={{ display:'flex', gap:10 }}>
            <div className="tabs">
              {[2024,2025].map(y => <button key={y} className={`tab ${fiscalYear===y?'active':''}`} onClick={() => setFiscalYear(y)}>{y}</button>)}
            </div>
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr>{['Member','Loan #','Principal','Interest','Deposited','Issued','Repaid','Balance','Status',''].map(h=><th key={h}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {list.map(l => (
              <tr key={l.id}>
          ...
          <form onSubmit={handleAddLoan} className="modal-form">
            ...
            {form.principal && (
              <div style={{ background:'var(--bg-input)', borderRadius:8, padding:'10px 14px', fontSize:12, color:'var(--text-muted)' }}>
                Interest (5%): <strong style={{ color:'var(--accent-amber)' }}>{fmt(Math.round(parseInt(form.principal||0)*0.05))}</strong>
                &nbsp;· Deposited: <strong style={{ color:'var(--accent-teal)' }}>{fmt(Math.round(parseInt(form.principal||0)*0.95))}</strong>
              </div>
            )}
            <div className="grid-2">
              <div className="form-group">
            ...
            <div className="modal-actions">
              ...
              <button type="submit" className="btn btn-primary" disabled={saving}>{saving?'Saving…':'Issue Loan'}</button>
            </div>
          </form>
        </Modal>
      )}
