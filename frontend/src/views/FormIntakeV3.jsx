  ...
  return (
    <div className="intake-final-overlay" onClick={(event) => event.target === event.currentTarget && !posting && onBack()}>
      <div className="intake-final-card">
        <div className="intake-final-head">
...
function PaymentReviewModal({ row, onClose, onChanged }) {
  ...
  const currentStep = !verificationComplete ? 1 : !allocationComplete ? 2 : 3;
  ...
  const saveVerification = async () => {
    ...
    try {
      ...
      showToast('Verification saved. The member’s live obligations have been loaded below.');
    ...
    <div className="admin-modal-backdrop" onClick={(event) => event.target === event.currentTarget && onClose()}>
      <div className="admin-modal-panel" style={{ maxWidth: 1080, maxHeight: '94vh', overflowY: 'auto' }}>
          ...
          <div style={{ padding: '0 20px 22px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 14, alignItems: 'start' }}>
              ...
              <section className="admin-table-card" style={{ padding: 16 }}>
                ...
                <div className="intake-guidance" style={{ marginBottom: 12 }}><ShieldCheck size={15} style={{ flexShrink: 0, marginTop: 1 }} /><span><strong>Important:</strong> selecting a member here does not load their obligations until you click <strong>Save verification & load obligations</strong>.</span></div>
                ...
                <button type="button" className="admin-btn-primary" style={{ marginTop: 12 }} onClick={saveVerification} disabled={saving}>{saving ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />}{saving ? 'Saving…' : verificationDirty || !verificationComplete ? 'Save verification & load obligations' : 'Reload verified obligations'}</button>
              </section>
            </div>
            ...
            <div style={{ marginTop: 20 }} className="intake-section-kicker">Step 2 · Allocate the funds</div>
            <div className="intake-guidance" style={{ marginBottom: 12 }}><CircleDollarSign size={15} style={{ flexShrink: 0, marginTop: 1 }} /><span><strong>This is where you allocate the money.</strong> Use the Add buttons under Monthly contributions, Existing unpaid fines, or Outstanding loans. Every item appears in <strong>Receipt allocation</strong> on the right. Adjust contribution or loan amounts there until <strong>Remaining = TZS 0</strong>.</span></div>

            {!verificationComplete && <div className="admin-rule-notice" style={{ marginBottom: 12, borderColor: '#fde68a', background: '#fffbeb' }}><AlertTriangle size={14} color="#d97706" /><span><strong>Step 1 is not saved yet.</strong> Save the verified member/reference above first. The obligation list below is intentionally locked so you do not allocate against the wrong account.</span></div>}

            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.15fr) minmax(330px,.85fr)', gap: 14, alignItems: 'start' }}>
              <section className={`admin-table-card${verificationComplete ? '' : ' intake-obligations-disabled'}`}>
                <div style={{ padding: 14, borderBottom: '1px solid var(--admin-border)' }}><strong style={{ fontSize: 13 }}>Verified member’s ledger obligations</strong><div style={{ fontSize: 11, color: 'var(--admin-muted)', marginTop: 3 }}>{verificationComplete ? `Loaded for ${verifiedMemberName}. Add only what this receipt actually settles.` : 'Save Step 1 to load the correct member’s obligations.'}</div></div>
                ...
                <div style={{ padding: 14, borderBottom: '1px solid var(--admin-border)' }}>
                  ...
                  <div style={{ display: 'grid', gap: 7 }}>
                    ...
                    {(workbench.contribution_candidates || []).length === 0 && <div className="intake-empty-state">No unpaid/current contribution periods were found for the saved verified member. If you just selected a different member above, save Step 1 first. If this member is correct, there is currently no contribution obligation available to receive this cash.</div>}
                  </div>
                </div>
        ...
        <div className="admin-modal-actions" style={{ position: 'sticky', bottom: 0, background: '#fff', zIndex: 5 }}>
          <div style={{ marginRight: 'auto', fontSize: 11, color: 'var(--admin-muted)' }}>{currentStep === 1 ? 'Save verification before allocating.' : currentStep === 2 ? `Allocate ${fmt(Math.max(remaining, 0))} more before review.` : 'Allocation is balanced. Review the final posting before committing.'}</div>
          <button type="button" className="admin-btn-secondary" onClick={onClose} disabled={posting}>Cancel</button>
          <button type="button" className="admin-btn-primary" onClick={() => setShowFinalReview(true)} disabled={posting || loading || previewing || !allocationComplete} title={!allocationComplete ? 'Complete Steps 1 and 2 first' : 'Review ledger entries before final posting'}><ArrowRight size={14} /> Review final posting</button>
        </div>
      </div>

      {showFinalReview && allocationComplete && <FinalPostingReview submission={submission} memberName={verifiedMemberName} allocations={preview?.allocations || allocations} received={received} posting={posting} onBack={() => setShowFinalReview(false)} onPost={postPayment} />}
    </div>
  );
}
