...
const { Loan, Repayment, Member, Transaction, Contribution } = require('../db/models');
...
async function enrichLoan(loan) {
  ...
  let penalty = 0;
  let months_active = getMonthsDiff(loan.issued_date, new Date());
  
  if (loan.status !== 'paid' && loan.fiscal_year >= 2026 && months_active > 6) {
    const months_overdue = months_active - 6;
    penalty = Math.round(loan.principal * 0.10 * months_overdue);
  }
...
router.get('/', authenticate, async (req, res) => {
  ...
  let list = await Loan.find(query).lean();
...
router.post('/', authenticate, requireAdmin, async (req, res) => {
  ...
  // Enforce Max Loan Cap (80% of total member contributions) for FY >= 2026
  if (fy >= 2026) {
  ...
  // Enforce 12% interest for FY >= 2026, otherwise 5%
  const interest_rate = fy >= 2026 ? 0.12 : 0.05;
  const interest_amount = Math.round(p * interest_rate);

  // Auto calculate due date if not provided (6 months for FY>=2026)
...
router.post('/:id/repayments', authenticate, requireAdmin, async (req, res) => {
  ...
  // Re-calculate balance dynamically using the enriched loader so we don't accidentally close early
  const allReps = await Repayment.find({ loan_id: id }).lean();
  const totalRepaid = allReps.reduce((s,r) => s+r.amount, 0);

  // Determine penalty if any
  let penalty = 0;
  let months_active = getMonthsDiff(loan.issued_date, new Date());
