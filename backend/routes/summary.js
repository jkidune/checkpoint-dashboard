...
const { Member, Contribution, Loan, Repayment, Fine, WelfareEvent, Transaction } = require('../db/models');
...
router.get('/', authenticate, async (req, res) => {
  ...
  // Calculate Entry Fees strictly (use actual db values vs defaulting to 100k)
  ...
  // Monthly contribution stats dynamically
  ...
  // Monthly stats dynamically compares all available years
  const monthly_stats = [];
  for (let month = 1; month <= 12; month++) {
    const obj = { month };
  ...
  // Interest by member securely & dynamically
...
router.post('/fines', authenticate, requireAdmin, async (req, res) => {
  ...
  const fine = new Fine({ member_id: parseInt(member_id), amount: parseInt(amount), reason: reason||'Late contribution', year: parseInt(year)||2026, status: status||'unpaid', paid_date: paid_date||null });
  await fine.save();
...
router.post('/welfare', authenticate, requireAdmin, async (req, res) => {
  ...
  const welfare = new WelfareEvent({
...
router.patch('/welfare/:id', authenticate, requireAdmin, async (req, res) => {
  ...
  const welfare = await WelfareEvent.findOne({ id });
  if (!welfare) return res.status(404).json({ error: 'Not found' });
  
  if (status) welfare.status = status;
  if (approved_date) welfare.approved_date = approved_date;
  
  if (welfare.status === 'approved' && !welfare.approved_date) {
    welfare.approved_date = new Date().toISOString().split('T')[0];
  }
  
  await welfare.save();

  // If approved, create a transaction deducting from group
  if (status === 'approved') {
