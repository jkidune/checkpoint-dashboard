...
// Fine eligibility is deliberately based on PRESENCE, not paid_date.
// Any contribution record for the member/month means the automatic fine scanner
// leaves that month alone. This protects historical records from delayed data entry
// and also avoids automatically fining partial contributions.
function hasContributionForPeriod(contributions, memberId, month, year) {
  return contributionRows(contributions, memberId, month, year).length > 0;
}
...
async function outstandingLoanBalance(loans, repayments, memberId, now, rulesCache) {
  ...
  for (const loan of loans.filter((row) => Number(row.member_id) === Number(memberId)
    ...
    && row.disbursed !== false)) {
    const issued = new Date(`${loan.issued_date || dateKey(now)}T12:00:00Z`);
    const loanFy = Number(loan.fiscal_year || getFiscalYear(
      issued.getUTCMonth() + 1,
...
async function createNotificationIfNew({ memberId, message, dueDate }) {
  ...
  const notification = await Notification.create({
    ...
    read: false,
    created_by: 'system:auto-missing-fines',
  });
...
async function logFineEmail({ member, batchKey, subject, status, info, failureReason, source }) {
  return CommunicationLog.create({
    ...
    source_entity_type: 'automatic_missing_contribution_fine_batch',
    source_entity_id: batchKey,
    created_by: source || 'system:auto-missing-fines',
  });
}

async function runAutomaticFineIssuance(options = {}) {
  const now = options.now instanceof Date ? options.now : new Date(options.now || Date.now());
  const source = options.source || 'system:auto-missing-fines';
  ...
  if (!periods.length) {
    return {
      ok: true,
      policy: 'missing-contribution-only',
      scanned_at: now.toISOString(),
      automation_start: '2026-03',
      fines_created: 0,
      members_notified: 0,
      message: 'No FY2026/2027-or-later contribution period has reached its fine-check date yet.',
      results: [],
    };
  }
  ...
  for (const member of members) {
    for (const period of periods) {
      ...
      // Critical rule: if any contribution record exists for this member/month,
      // automatic fine issuance stops here. paid_date is intentionally ignored.
      if (hasContributionForPeriod(contributions, member.id, period.month, period.year)) {
        scanDetails.push({
          member_id: member.id,
          month: period.month,
          year: period.year,
          action: 'skipped_contribution_exists',
        });
        continue;
      }
      ...
      if (existingPeriodFine) {
        scanDetails.push({
          ...
          fine_status: existingPeriodFine.status,
        });
        continue;
      }
      ...
      try {
        fine = await Fine.create({
          ...
          notes: `Automatically issued after ${dateKey(period.deadline)} because no contribution record existed for ${periodLabel(period.month, period.year)}. Recorded payment dates are not used for fine eligibility.`,
        });
      } catch (error) {
        if (error?.code === 11000) {
          scanDetails.push({
      ...
      scanDetails.push({
        ...
        month: period.month,
        year: period.year,
        action: 'fine_created_missing_contribution',
  ...
  for (const member of members) {
    ...
    const fineLines = memberFines.map((fine) =>
      `${periodLabel(fine.contribution_month, fine.contribution_year)} — fine TZS ${Number(fine.amount || 0).toLocaleString('en-US')} (no contribution recorded)`);

    const notificationMessage = `${memberFines.length === 1 ? 'Missing contribution fine issued' : 'Missing contribution fines issued'}: ${fineLines.join('; ')}. Total currently owed to the club: TZS ${totalOwed.toLocaleString('en-US')}.`;
    ...
    const batchKey = `auto-missing-fines:${member.id}:${memberFines
    ...
    if (!member.email) {
      ...
      if (alreadySent) {
        ...
        const subject = memberFines.length === 1
          ? `Checkpoint missing contribution fine — ${periodLabel(memberFines[0].contribution_month, memberFines[0].contribution_year)}`
          : `Checkpoint missing contribution fines — ${memberFines.length} months`;
        const message = [
          `Checkpoint found ${memberFines.length === 1 ? 'a contribution month with no contribution recorded' : `${memberFines.length} contribution months with no contribution recorded`} after the applicable deadline.`,
          '',
          ...
          '',
          'Fine automation checks only whether a contribution record is missing after the deadline. The recorded contribution/payment date is not used to create a fine.',
          '',
          'If you already made a contribution that is not yet visible in Checkpoint, send the payment reference to the administrator for verification.',
        ].join('\n');
    ...
    results.push({
      ...
      fines_issued: memberFines.map((fine) => ({
        ...
        trigger: 'no_contribution_record',
      })),
  ...
  return {
    ...
    recorded_contribution_dates_ignored_for_fines: true,
    partial_or_existing_contribution_rows_not_auto_fined: true,
