// Default/legacy connection compatibility layer for Form Intake
...
function createFormIntakeSchemas() {
  ...
  return { formIntakeSubmissionSchema };
}

/**
 * @param {import('mongoose').Connection} connection
 */
function bindFormIntakeModels(connection) {
  const { formIntakeSubmissionSchema } = createFormIntakeSchemas();

  const FormIntakeSubmission =
    connection.models.FormIntakeSubmission ||
    connection.model('FormIntakeSubmission', formIntakeSubmissionSchema);

  return { FormIntakeSubmission };
}

const defaultModels = bindFormIntakeModels(mongoose.connection);

module.exports = {
  ...defaultModels,
  bindFormIntakeModels,
};
