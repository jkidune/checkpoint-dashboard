...
- **Phase 4 — route-by-route tenant model migration: in progress (Phase 4B this batch).** `backend/routes/members.js`, `transactions.js`, `expenses.js` (Phase 4A) and `backend/routes/memberLoanEligibility.js` (Phase 4B) now read/write exclusively through `req.tenantModels`, with zero fallback to the default/legacy model registry. Every other financial/public/background route is still default-bound and explicitly not migrated yet — see [Phase 4A migration status](#18-phase-4a-migration-status), [Phase 4B migration status](#19-phase-4b-migration-status), and the full inventory in `docs/tenant-route-migration.md`. **The application is not fully multi-tenant** — the single-runtime-tenant restriction from Phase 3 still applies and remains load-bearing.
